...
//Para poder gestionar los registros de Persona
package datos;
...
public class PersonaDAO implements CrudPaginadoInterface<Persona> { 
    ...
    public List<Persona> listar(String texto,int totalPorPagina,int numPagina) {
        ...
        try {
          ps=CON.conectar().prepareStatement("SELECT p.id, p.tipo_persona, p.nombre, p.tipo_documento, p.num_documento, p.direccion, p.telefono, p.email, p.activo FROM persona p WHERE p.nombre LIKE ? ORDER BY p.id ASC LIMIT ?,?");
            ...
            ps.setInt(2, (numPagina-1)*totalPorPagina); //limitar registros a mostrar, desde que registro empezar a mostrar
            ps.setInt(3, totalPorPagina);//registros mostrar a partir del registro anterior 
            rs=ps.executeQuery();
            while(rs.next()){
     ...
     public List<Persona> listarTipo(String texto,int totalPorPagina,int numPagina, String tipoPersona) {
        ...
        try {
          ps=CON.conectar().prepareStatement("SELECT p.id, p.tipo_persona, p.nombre, p.tipo_documento, p.num_documento, p.direccion, p.telefono, p.email, p.activo FROM persona p WHERE p.nombre LIKE ? AND tipo_persona=? ORDER BY p.id ASC LIMIT ?,?");
            //PreparedStatement envia 4 parametros
            ps.setString(1,"%" + texto +"%"); //parametro like  
            ps.setString(2, tipoPersona); //parametro tipo persona
            ps.setInt(3, (numPagina-1)*totalPorPagina); //par para limitar cantidad de registros a mostrar, en este caso desde que registro empezar a mostrar
            ps.setInt(4, totalPorPagina);//par para saber cuantos registros mostrar a partir del registro anterior 
            rs=ps.executeQuery();
            while(rs.next()){
                registros.add(new Persona(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getBoolean(9)));
            }
    ...
    @Override
    public boolean insertar(Persona obj) {//Recibe un tipo Persona
        resp=false;
        try {
            ...
            //enviamos al ps los valores, obtenidos de cada uno de los atributos del obj que instancia a Articulo
    ...
    public boolean actualizar(Persona obj) {
        ...
        try {
            ...
            ps.setString(6, obj.getTelefono());
            ps.setString(7, obj.getEmail());
            ps.setInt(8,obj.getId()); //enviamos también el ID
            if (ps.executeUpdate()>0){
                resp=true;
            }
            ps.close();
